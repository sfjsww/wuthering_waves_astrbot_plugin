...
class GachaMixin:
    ...
    async def query_waves_gacha_records(self, event: AstrMessageEvent, card_pool_type: str = "", uid: str = None):
        ...
        data = await self.kuro.get_gacha(query)
        if data["status"]:
            img = await self.render.render("gacha/gacha", {"gachaData": data["data"]})
    ...
    async def waves_manage_gacha_records(self, event: AstrMessageEvent, action: str, data: str = None):
        ...
        if action == "export":
            ...
            if not tokens:
                ...
                return
            account = tokens[0]
            cached = self.config_mgr.get_gacha_records(account.get("roleId", ""))
            if cached:
                import json
