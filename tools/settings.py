...
def register_settings_tool(plugin):
    ...
    async def waves_update_settings(self, event: AstrMessageEvent, setting: str, enabled: bool, threshold: int = None):
        '''更新鸣潮插件用户设置。Args: setting(string): "auto_sign"/"auto_task"/"sanity_push"/"news_push", enabled(bool): 开启或关闭, threshold(int, optional): 体力推送阈值'''
        user_id = event.get_sender_id()
        if setting == "sanity_push" and threshold:
            plugin.config_mgr.set_config("sanity_threshold", threshold)
    ...
    plugin.register_tool("waves_update_settings", waves_update_settings, "_tool_settings")
