        ...
        if action == "sign":
            ...
            for account in tokens:
                ...
                if not ok:
                    ...
                    continue
                sign = await self.kuro.sign_in(account["serverId"], account["roleId"], account.get("userId", account["roleId"]), account["token"])
                rec = await self.kuro.query_record(account["serverId"], account["roleId"], account["token"])
            ...
            for account in tokens[:1]:
                ...
                if not ok:
                    ...
                    continue
                record = await self.kuro.query_record(account["serverId"], account["roleId"], account["token"])
                if record["status"]:
                    record["data"] = record["data"][:50]
                    img = await self.render.render("queryRecord/queryRecord", {"listData": record["data"]})
