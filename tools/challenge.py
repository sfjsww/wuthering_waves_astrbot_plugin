"""query_waves_challenge_data - 对标 apps/Challenge.js"""
from astrbot.api.event import filter, AstrMessageEvent

def register_challenge_tool(plugin):
    ...
    async def query_waves_challenge_data(self, event: AstrMessageEvent, uid: str = None):
        ...
        if not ok:
            ...
            return
        data = await plugin.kuro.get_challenge_data(account["serverId"], account["roleId"], account["token"])
        if data["status"]:
            img = await plugin.render.render("challengeDetails/challengeDetails", {"data": data["data"]})
            yield event.image_result(img)
        else:
            yield event.plain_result(data["msg"])
    plugin.register_tool("query_waves_challenge_data", query_waves_challenge_data, "_tool_challenge")
