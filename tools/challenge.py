...
class ChallengeMixin:
    ...
    async def query_waves_challenge_data(self, event: AstrMessageEvent, uid: str = None):
        ...
        if not ok:
            ...
            return
        data = await self.kuro.get_challenge_data(account["serverId"], account["roleId"], account["token"])
        if data["status"]:
            img = await self.render.render("challengeDetails/challengeDetails", {"data": data["data"]})
