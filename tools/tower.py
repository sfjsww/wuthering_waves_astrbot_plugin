"""query_waves_tower - 对标 apps/Tower.js"""
from astrbot.api.event import filter, AstrMessageEvent

def register_tower_tool(plugin):
    ...
    async def query_waves_tower(self, event: AstrMessageEvent, uid: str = None):
        ...
        if not ok:
            ...
            return
        data = await plugin.kuro.get_tower_data(account["serverId"], account["roleId"], account["token"])
        if data["status"]:
            img = await plugin.render.render("towerData/towerData", {"data": data["data"]})
            yield event.image_result(img)
        else:
            yield event.plain_result(data["msg"])
    plugin.register_tool("query_waves_tower", query_waves_tower, "_tool_tower")
