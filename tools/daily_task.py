...
def register_daily_task_tool(plugin):
    ...
    async def waves_daily_task(self, event: AstrMessageEvent, action: str = "list"):
        ...
        if action == "do":
            ...
            if data["status"]:
                img = await plugin.render.render("taskList/taskList", {"data": data["data"]})
                yield event.image_result(img)
            else:
                yield event.plain_result(data["msg"])
        else:
            yield event.plain_result("请指定 action: do(执行任务) / list(任务列表)")
    plugin.register_tool("waves_daily_task", waves_daily_task, "_tool_daily_task")
